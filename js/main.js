// --- DATA ---
 ...
 async function loadContent(filePath, containerId) {
     try {
         ...
         const text = await response.text();         document.getElementById(containerId).innerHTML = text;
     } catch (error) {
...
// --- INITIALIZATION FUNCTIONS ---
     ...
     if (slides.length > 0 && prevBtn && nextBtn) {
        ...
        function startAutoplay() {
         ...
         prevBtn.addEventListener('click', () => {
             stopAutoplay(); // Stop autoplay when user takes control             plusSlides(-1);
         });
 ...
 function initializeEventsPage() {
    ...
    document.getElementById('event-detail-view').style.display = 'none';
     ...
     document.querySelectorAll('#past-events-list li').forEach(item => {         item.addEventListener('click', (event) => {
